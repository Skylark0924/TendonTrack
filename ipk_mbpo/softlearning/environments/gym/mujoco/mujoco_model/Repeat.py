...
def my_composite(parent_pos, body_type, spacing, count, segement, size, site_size, rgba, damping, stiffness, density):
    if count <= 1:
        return -1
    ...
    with io.open(filename, 'w') as f:
        for i in range(count):
            ...
            pos = i * spacing
            geom_txt = "<geom type=\"%s\" pos=\"0 0 %.4f\" size=\"%s\" density=\"%d\" rgba=\"%s\"/>\n" \
                       % (body_type, pos, size, density, rgba)
