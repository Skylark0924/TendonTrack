...
def detect_contor(img):
    ...
    for c in contours:
        ...
        if DEBUG_FLG:
            print(cX_tmp, cY_tmp, cArea)
        # 画出轮廓和中点
        cv2.drawContours(img, [c], -1, (0, 255, 0), 2)
        cv2.circle(img, (cX_tmp, cY_tmp), 7, (255, 255, 255), -1)
        cv2.putText(img, 'Center:{},{} Area:{}'.format(cX_tmp-width//2, cY_tmp-height//2, cArea), (cX_tmp - 20, cY_tmp - 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
        cXY.append([cX_tmp, cY_tmp, cArea])
        # 显示图像
        if DEBUG_FLG:
            cv2.imshow("Image", img)
            cv2.waitKey(2)
...
if __name__ == "__main__":
    img = cv2.imread('./tmp_1.jpg')  # /home/lab/Github/TendonTrack/Simulator/utils/image_log
    # seg_bg_fg(img)
    detect_contor(img)
