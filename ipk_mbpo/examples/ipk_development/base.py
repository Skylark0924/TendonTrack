...
POLICY_PARAMS_BASE = {
    'IPKGaussianPolicy': GAUSSIAN_POLICY_PARAMS_BASE,
}

POLICY_PARAMS_BASE.update({
    'gaussian': POLICY_PARAMS_BASE['IPKGaussianPolicy'],
})

POLICY_PARAMS_FOR_DOMAIN = {
    'IPKGaussianPolicy': GAUSSIAN_POLICY_PARAMS_FOR_DOMAIN,
}

POLICY_PARAMS_FOR_DOMAIN.update({
    'gaussian': POLICY_PARAMS_FOR_DOMAIN['IPKGaussianPolicy'],
})
...
MAX_PATH_LENGTH_PER_DOMAIN = {
    ...
    'Pendulum': 200,
    'Tendon': 200,
    'Tendon_Tracking': 300,  # timestep per episode
}
...
ALGORITHM_PARAMS_ADDITIONAL = {
    'IPK': {
        ...
        'kwargs': {
            ...
            'n_initial_exploration_steps': int(600),  # 初始化 pool 需要的总 sample 数
        }
    },
    'MBPO': {
        'type': 'MBPO',
...
def get_variant_spec_ipk(universe, domain, task, policy, algorithm, env_params):
    algorithm_params = deep_update(
        env_params,
        ALGORITHM_PARAMS_PER_DOMAIN.get(domain, {})
    )
    ...
    variant_spec = {
        ...
        'algorithm_params': algorithm_params,
        'replay_pool_params': {
            'type': 'IPKSimpleReplayPool',
            'kwargs': {
                'max_size': tune.sample_from(lambda spec: (
                    {
                        'IPKSimpleReplayPool': int(1e6),
                        'TrajectoryReplayPool': int(1e4),
                    }.get(
                        spec.get('config', spec)
                        ['replay_pool_params']
...
def get_variant_spec(args, env_params):
    ...
    if env_params.type == 'MBPO':
        variant_spec = get_variant_spec_base(
            universe, domain, task, args.policy, env_params.type, env_params)
    elif env_params.type == 'IPK':
        variant_spec = get_variant_spec_ipk(
            universe, domain, task, args.policy, env_params.type, env_params)
