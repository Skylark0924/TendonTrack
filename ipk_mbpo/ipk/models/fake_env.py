...
class FakeEnv:
    ...
    def step(self, obs, act, deterministic=False):
        ...
        # print('log_prob: {}, dev: {}'.format(log_prob, dev))

        rewards, next_obs = samples[:, :1], samples[:, 1:]
        terminals = self.config.termination_fn(obs, act, next_obs)
        # print('rewards: {}, next_obs: {}, terminals: {}'.format(rewards, next_obs, terminals))
    ...
    def step_ph(self, obs_ph, act_ph, deterministic=False):
        ...
        ensemble_model_means = tf.concat(
            [ensemble_model_means[:, :, 0:1], ensemble_model_means[:, :, 1:] + obs_ph[None]], axis=-1)
        ...
        if deterministic:
            ...
            ensemble_samples = ensemble_model_means + tf.random.normal(
                tf.shape(ensemble_model_means)) * ensemble_model_stds
