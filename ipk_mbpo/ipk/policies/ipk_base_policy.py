...
'''
Basic structure of MBPO policy, the father class
'''
...
class LatentSpacePolicy(BasePolicy):
    def __init__(self, *args, smoothing_coefficient=None, **kwargs):
        ...
        self.ipk_basic_action = np.zeros((4,))
    ...
    def actions_np(self, conditions):
        import pdb
        # print('gaussian_action_conditions:{},{}'.format(conditions, type(conditions)))
        self.ipk_basic_action = self.ipk_basic_policy.action_np(conditions)
        print(' [ ipk_basic_action ]: {}'.format(self.ipk_basic_action))
        if self._deterministic:
            print('Houston! This is deterministic_actions_model!')

            return self.deterministic_actions_model.predict(conditions) \
                , self.kal_deterministic_actions_model.predict(
            [*conditions, self.ipk_basic_action])
        elif self._smoothing_alpha == 0:
            # pdb.set_trace()
            # self.kalman_filter.get_ipk_basic_act(ipk_basic_action)
            return self.actions_model.predict(conditions) \
                , self.kal_actions_model.predict([*conditions, self.ipk_basic_action])
        else:
            print('Houston! This is actions_model_for_fixed_latents!')
            # self.kalman_filter.get_ipk_basic_act(ipk_basic_action)
            ...
            return self.actions_model_for_fixed_latents.predict([*conditions, latents]) \
                , self.kal_actions_model_for_fixed_latents.predict(
            [*conditions, latents, self.ipk_basic_action])
